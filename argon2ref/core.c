...
void finalize(const argon2_context *context, argon2_instance_t *instance) {
    if (context != NULL && instance != NULL) {
        ...
        /* Hash the result */
        {
            ...
            ablake2b_long(context->out, context->outlen, blockhash_bytes,ARGON2_BLOCK_SIZE);
...
void initial_hash(uint8_t *blockhash, argon2_context *context,
    ...
    ablake2b_init(&BlakeHash, ARGON2_PREHASH_DIGEST_LENGTH);

    store32(&value, context->lanes);
    ablake2b_update(&BlakeHash, (const uint8_t *)&value, sizeof(value));
    ...
    if (context->pwd != NULL) {
        ablake2b_update(&BlakeHash, (const uint8_t *)context->pwd,
                       context->pwdlen);
    ...
    if (context->salt != NULL) {
        ablake2b_update(&BlakeHash, (const uint8_t *)context->salt,
                       context->saltlen);
    }
    ...
    if (context->secret != NULL) {
        ablake2b_update(&BlakeHash, (const uint8_t *)context->secret,
                       context->secretlen);
    ...
    if (context->ad != NULL) {
        ablake2b_update(&BlakeHash, (const uint8_t *)context->ad,
                       context->adlen);
    }

    ablake2b_final(&BlakeHash, blockhash, ARGON2_PREHASH_DIGEST_LENGTH);
